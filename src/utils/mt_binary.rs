...
impl<F: RichField, H: Hasher<F>> PartialMT<F, H> {
    ...
    pub fn update_tree_mul(&mut self, updates: BTreeMap<u32, Vec<F>>) {
        ...
        for i in 1..self.log_max_capacity + 1 as u8 {
            leaf_indices = BTreeSet::from_iter(leaf_indices.into_iter().map(|i| i >> 1));
    ...
    pub fn from_bucket_proofs(proofs: &BTreeMap<u32, (Vec<F>, MerkleBucketProof<F, H>)>) -> Self {
        let mut level = 0 as u8;
        let mut digests: BTreeMap<(u8, u32), H::Hash> = BTreeMap::new();
        let mut visited_bucket = BTreeSet::<usize>::new();
        for (k, v) in proofs {
...
mod tests {
    ...
    fn mt_test_base<F: RichField, H: Hasher<F>>(ell: u8, log_subset_size: u8) {
        ...
        let subset_indices: BTreeSet<u32> =
            BTreeSet::from_iter((0..size).choose_multiple(&mut r, subset_size as usize));
